function FastEstimation(sig,xname)

%% definitions

figure('name', 'linear scale');
%% Fast estimation more information see husrt_matt.m
...
[logx,logy]=fastdfa_core(sig);

plot(logx,logy);

hfig=figure('name',['FAST DFA' xname]);

set(hfig,'color',[0.8 0.8 0.8],'name','FAST DFA',...
...
uicontrol('Units','Normalized','Position',[0.625,0.8,0.15,0.1],...
    'String','FAST DFA','Style','text','BackgroundColor',...
    [0.93 0.93 0.93]);
...
uicontrol('Units','Normalized','Position',[0.625,0.675,0.15,0.1],...
    ...
    function cal_callback(source, eventdata)
        ...
        %Note funtion fastdfa_core calculates the scaling exponent so 
        %to find the hurst it is done the next formula
        hurst=p(1);
        ...
        for i=1:10
            [logxa,logya]=fastdfa_core(xaa(:,i));
             xroi=log10(logxa(ind));
             yroi=log10(logya(ind));
             p=polyfit(xroi,yroi,1);
        %Note funtion fastdfa_core calculates the scaling exponent so 
        %to find the hurst it is done the next formula
             hurstAAFT(i)=p(1);       
...
%         % white noise
%         wn=rand(size(sig,1),10);
%         hurstwn=zeros(1,10);
%         for i=1:10
%             [logxwn,logywn]=fastdfa_core(wn(:,i));
%             xroi=log10(logxwn(ind));
%             yroi=log10(logywn(ind));
%             p=polyfit(xroi,yroi,1);
%             hurstwn(i)=p(1);
%         end
%         
...
%         hurstrw=zeros(1,10);
%         for i=1:10
%             [logxrw,logyrw]=fastdfa_core(rw(:,i));
%             xroi=log10(logxrw(ind));
%             yroi=log10(logyrw(ind));
%             p=polyfit(xroi,yroi,1);
%             hurstrw(i)=p(1);
%         end
%         
...
%             'ForegroundColor',gor,'visible','on');
        ...
        [logxrw,logyrw]=fastdfa_core(diff(rw));
        xroi=log(logxrw);
        yroi=log(logyrw);
        p=polyfit(xroi,yroi,1);
        hurstrw=p(1);
        
        set(trw,'string',num2str(hurstrw),'visible','on');
