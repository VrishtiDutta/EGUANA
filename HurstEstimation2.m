function HurstEstimation2(sig,xname)
...
uicontrol('Units','Normalized','Position',[0.625,0.8,0.15,0.1],...
    'String','Hurst Estimation 2','Style','text','BackgroundColor',...
    [0.93 0.93 0.93]);
...
twnt=uicontrol('Units','Normalized','Position',[0.625,0.3,0.15,0.1],...
    'String','White Noise','Style','text','BackgroundColor',...
    [0.93 0.93 0.93],'visible','off');


trwt=uicontrol('Units','Normalized','Position',[0.625,0.425,0.15,0.1],...
    'String','Random Shuffle','Style','text','BackgroundColor',...
    [0.93 0.93 0.93],'visible','off');
...
twn=uicontrol('Units','Normalized','Position',[0.8,0.3,0.1,0.1],...
    'String','Hurst Exponent','Style','text','BackgroundColor',...
    [0.93 0.93 0.93],'visible','off');

trw=uicontrol('Units','Normalized','Position',[0.8,0.425,0.1,0.1],...
    'String','Hurst Exponent','Style','text','BackgroundColor',...
    [0.93 0.93 0.93],'visible','off');
...
%         haahighconf= hurstAAFTZinv+(1.96*hurstAAFTZsd);
        ...
        if( hurst < max(hurstAAFT) && hurst > min(hurstAAFT) )
            gor='g';
        else
            gor='r';
        end

        set(taaft,'string',[num2str(min(hurstAAFT)),' ',num2str(max(hurstAAFT))],...
...
%             'ForegroundColor',gor,'visible','on');
        ...
        hurstwn=zeros(10,1);
        for i=1:10
            wn=2*rand(size(sig))-1;
            %hurstwn=zeros(1,10);
            [x,y,H]=hurst_rafael(wn);
            logxwn=log(x);
            logywn=log(y);
            xroi=logxwn;
            yroi=logywn;
        ...
        if( hurst < max(hurstwn) && hurst > min(hurstwn) )
            gor='g';
        else
            gor='r';
        end
       
        set(twn,'string',[num2str(min(hurstwn)) ' ' num2str(max(hurstwn))],...
                'ForegroundColor',gor,'visible','on');
        
        % randow sig
        hurstdes=zeros(10,1);
        for i=1:10
            [sorted,indtodesort]=sort(rand(size(sig)));
            sigdesort=sig(indtodesort);
            [x,y,H]=hurst_rafael(sigdesort);
            logxdes=log(x);
            logydes=log(y);
            xroi=logxdes(ind);
            yroi=logydes(ind);
        ...
        if( hurst < max(hurstdes) && hurst > min(hurstdes) )
            gor='g';
        else
            gor='r';
        end

        set(trw,'string',[num2str(min(hurstdes)) ' ' num2str(max(hurstdes))],...
            'ForegroundColor',gor,'visible','on');
    end
