function varargout = gui_rqa(varargin)
% GUI_RQA M-file for gui_rqa.fig
%      GUI_RQA, by itself, creates a new GUI_RQA or raises the existing
%      singleton*.
%
%      H = GUI_RQA returns the handle to a new GUI_RQA or the handle to
%      the existing singleton*.
%
%      GUI_RQA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_RQA.M with the given input arguments.
%
%      GUI_RQA('Property','Value',...) creates a new GUI_RQA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before gui_hurst_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to gui_rqa_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help gui_rqa

% Last Modified by GUIDE v2.5 02-Sep-2010 11:54:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @gui_rqa_OpeningFcn, ...
                   'gui_OutputFcn',  @gui_rqa_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before gui_rqa is made visible.
function gui_rqa_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to gui_rqa (see VARARGIN)
...
% Choose default command line output for gui_rqa
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes gui_rqa wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = gui_rqa_OutputFcn(hObject, eventdata, handles) 
...
Shufopt1=input('Do you want to see RQA data from shuffled version? y(es) n(o)','s');
if Shufopt1 == 'y'
...
if isempty(handles.path) % test data
    ...
    if path~=0
       ...
       fprintf(fid, '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t\n', handles.filename, handles.subname, num2str(handles.trialnum),... 
           num2str(handles.delay1), num2str(handles.embed1), num2str(handles.radius1), ...
            ...
            num2str(handles.meanline1), num2str(handles.trend1));
